...
def dijkstra_targets(product, prod_source, prod_targets, avoid_nodes=[]):
    ...
    for prod_accep in prod_targets:
        ...
        if accept_pre_set:
            for accept_pre in accept_pre_set:
                if accept_pre not in avoid_nodes:
...
def dijkstra_targets_reverse(product, prod_source, prod_targets, avoid_nodes=[]):
    # for reverse product graph only, shortest path from source to a set of targets
    target = [prod_source]
    for source in prod_targets:
        for path, cost in dijkstra_targets(product, source, target, avoid_nodes):
            yield list(reversed(path)), cost
